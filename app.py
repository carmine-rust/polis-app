...
# 2. LOGICA ANTI-CRASH: Carichiamo il CSS un pezzo alla volta
st.markdown('<style>html, body { font-family: "Lato", sans-serif; }</style>', unsafe_content_allowed=True)
st.markdown('<style>.stApp { background-color: #001d3d; color: white; }</style>', unsafe_content_allowed=True)
st.markdown('<style>h1, h2, h3 { color: #00b4d8 !important; }</style>', unsafe_content_allowed=True)
...
st.subheader("Configuratore Professionale 2026")
...
# --- LOGICA DI CALCOLO (CON LIMITATORE Pd) ---
p_att = 0.0
p_new = 0.0
dist_m = 0
c_tec = 0.0
...
if pratica == "Spostamento Misuratore":
    tipo_spost = st.radio("Distanza", ["Entro 10m", "Oltre 10m"])
    ...
    # Fattori limitatore (1.1 in BT fino a 30kW)
...
with r1:
    st.markdown(f"""
    <div style="background:#003566; padding:20px; border-radius:10px; border:1px solid #00b4d8">
        <h4>Dettaglio Tecnico</h4>
        Potenza Disponibile: {(p_new * f_new):.2f} kW<br>
        Quota TIC: {c_tec:.2f} €<br>
        Oneri Gestione: {c_gest:.2f} €<br>
        Istruttoria: {TIC_2026['ISTRUTTORIA']:.2f} €
    </div>
    """, unsafe_content_allowed=True)

with r2:
    st.metric("TOTALE", f"{tot_finale:.2f} €")
    if is_split: st.caption("IVA in Split Payment")
