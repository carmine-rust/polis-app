...
st.caption("Configuratore Professionale v71.4 - Logica Spostamento vs Potenza")
...
with col2:
    ...
    p_new, p_att, f_new, dist_spostamento = 0.0, 0.0, 1.0, ""

    if is_spostamento:
        dist_spostamento = st.radio("Distanza Spostamento", ["Entro 10 metri", "Oltre 10 metri"], horizontal=True)
...
v1.metric("Tipo Quota", "Fissa (Spost.)" if is_spostamento else f"{p_new * f_new:.2f} kW")
...
def genera_pdf():
    ...
    # Riga 1 DINAMICA (Spostamento o Potenza)
    pdf.cell(140, 9, f" {desc_riga_1}", 1)
    ...
    pdf.cell(140, 9, f" IVA ({int(aliq*100)}%)", 1); pdf.cell(50, 9, f"{tot_iva:.2f} EUR", 1, 1, 'R')
    
    pdf.set_fill_color(240, 240, 240); pdf.set_font("Helvetica", "B", 11)
    pdf.cell(140, 11, " TOTALE DOVUTO", 1, 0, 'L', True); pdf.cell(50, 11, f"{tot_finale:.2f} EUR", 1, 1, 'R', True)
    
    # Condizioni e Firma
    pdf.ln(10); pdf.set_font("Helvetica", "B", 9); pdf.cell(0, 5, "CONDIZIONI DI ESECUZIONE:", ln=True)
    pdf.set_font("Helvetica", "", 9)
    testo_condizioni = (
        "- Conferma entro 30 gg;\n- Pagamento tramite bonifico: IT80P0103015200000007044056 - MPS\n"
...
if st.button("🚀 GENERA PDF"):
    ...
    else: st.warning("Inserisci Nome e Indirizzo!")
