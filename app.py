...
# 1. SETUP - SOLO COMANDI PYTHON NATIVI ALL'INIZIO
st.set_page_config(page_title="PolisEnergia Suite", page_icon="⚡", layout="wide")

# 2. COSTANTI TECNICHE TIC 2026
TIC_2026 = {
...
with r1:
    st.info(f"Potenza Disponibile Calcolata: **{(p_new * f_new):.2f} kW**")
...
# --- 3. INIEZIONE STILE SOLO ALLA FINE (Previene il crash del parser) ---
st.markdown('<style>html, body, .stApp { background-color: #001d3d; color: white !important; font-family: "Lato", sans-serif; } h1, h2, h3 { color: #00b4d8 !important; }</style>', unsafe_content_allowed=True)
